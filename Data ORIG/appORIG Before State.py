...
def get_namesgiveback():
    conn = sqlite3.connect('aep2.db')
    cursor = conn.cursor()
    cursor.execute("SELECT id, PlanName, PlanID FROM aeptbl WHERE PlanName LIKE '%Giveback%'")
    names = cursor.fetchall()
    conn.close()
    return names
...
def compare_selectgiveback():
    plans = get_namesgiveback()  # You already have this function: returns (id, PlanName, PlanID)
    return render_template('compare_selectgiveback.html', plans=plans)
...
def compare_results():
    ...
    if len(selected_ids) > 3:
        return "Please select up to 3 different plans.", 400
...
def show_select_carriers():
    ...
    if not selected_carriers:
        return "<p>No carriers selected. Please go back and choose at least one.</p>"  # No selection case
    ...
    query = f"SELECT * FROM aeptbl WHERE Carrier IN ({placeholders}) Order by Carrier"
...
def compare_all_plans():
    plans = get_names()  # You already have this function: returns (id, PlanName, PlanID)
    return render_template('compare_select_all.html', plans=plans)

@app.route('/compare_results_all', methods=['POST'])
def compare_resultsall():
    selected_ids = request.form.getlist('plans')
   # print("🔍 Selected plan IDs from form:", selected_ids)
    ...
    full_carrier_list = get_full_list()
   # print("🔍 Sample plan row:", full_carrier_list[0])

    # Filter full list based on selected plan IDs (assuming plan ID is at index 4)
    filtered_plans = [plan for plan in full_carrier_list if plan[4] in selected_ids_set]
    #print("All Plan IDs in full_carrier_list:", [str(plan[4]) for plan in full_carrier_list])
